...
const timeInputClassName =
  "mt-2 min-h-11 w-full rounded-xl border border-[var(--color-border)] bg-white px-2 text-base font-normal text-[var(--color-foreground)] outline-none transition-colors focus:border-[var(--color-brand)] focus:ring-3 focus:ring-[rgb(20_125_115/12%)]";
...
export function WeeklyAvailabilityForm({
  ...
  const { value: availability, setDraft: setAvailability, discardDraft, restored } = useConfigurationDraft("availability", initialAvailability, state);
  ...
  function toggleDay(dayOfWeek: number, active: boolean) {
    if (active) {
      setExpandedDays((current) => [...current, dayOfWeek]);
      replaceAvailability([
        ...availability,
  ...
  function copyDay(sourceDay: number) {
    ...
    setLastCopy({previous: availability, message: `Horarios copiados de ${days.find(day => day.value === sourceDay)?.label} a ${days.find(day => day.value === targetDay)?.label}.`});
  }
      ...
      <fieldset disabled={pending} className="m-0 min-w-0 border-0 p-0">
        ...
        <div className="mt-6 flex flex-col gap-3">
          {days.map((day) => {
            ...
            const expanded = expandedDays.includes(day.value) || Boolean(dayError);
            const replacesTarget = availability.some(block => block.dayOfWeek === copyTarget);
            const targetLabel = days.find(candidate => candidate.value === copyTarget)?.label;
            ...
            return (
              ...
              >
                <div className="flex min-h-16 items-center gap-3 px-4">
                  <label className="grid min-h-11 w-11 shrink-0 cursor-pointer place-items-center">
                    <input
                  ...
                  {active ? (
                    <button
                      aria-label={`${expanded ? "Cerrar" : "Editar"} horarios de ${day.label}`}
                      aria-expanded={expanded}
                      aria-controls={`hours-${day.value}`}
                      className="flex min-h-16 min-w-0 flex-1 items-center gap-2 rounded-xl border-0 bg-transparent py-2 text-left text-[var(--color-foreground)]"
                      onClick={() => setExpandedDays(current => expanded ? current.filter(value => value !== day.value) : [...current, day.value])}
                      type="button"
                    >
                    ...
                    <div className="min-w-0 flex-1 py-2">
                      <span className="block font-bold">{day.label}</span>
                      <span className="block text-sm text-[var(--color-muted)]">No atiendo</span>
                    </div>
                  )}
                </div>

                {active ? (
                  <div hidden={!expanded} id={`hours-${day.value}`} className="border-t border-[var(--color-border)] bg-[rgb(255_255_255/72%)] px-3 py-4 sm:px-4">
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <button
                              ...
                              className="inline-flex min-h-11 items-center gap-2 rounded-xl border-0 bg-transparent px-2 text-sm font-semibold text-[var(--color-muted)] hover:bg-[var(--color-warning-soft)] hover:text-red-700"
                          ...
                          <div className="mt-2 grid gap-2 min-[380px]:grid-cols-2">
                            <label className="min-w-0 text-sm font-medium text-[var(--color-muted)]">
                              Desde
                              <input
                    ...
                    <details className="group mt-4 border-t border-[var(--color-border)] pt-3">
                      ...
                      <p className="mt-2 mb-0 text-sm leading-5 text-[var(--color-muted)]">
                        {replacesTarget ? `Se reemplazarán los horarios de ${targetLabel}.` : `Se agregarán estos bloques a ${targetLabel}.`}
                      </p>
                      <div className="mt-2 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
                        <select
                          aria-label={`Copiar horarios de ${day.label} a`}
                          className="min-h-11 min-w-0 rounded-xl border border-[var(--color-border)] bg-white px-3 text-base"
                          onChange={(event) =>
                        ...
                        >
                          <Copy aria-hidden="true" size={16} />
                          {replacesTarget ? "Reemplazar" : "Copiar"}
                        </button>
                      </div>
                    </details>
        ...
        {lastCopy ? <div className="mt-4 rounded-xl bg-[var(--color-brand-subtle)] p-3">
          <p className="m-0 text-sm" role="status">{lastCopy.message}</p>
          <button type="button" className="min-h-11 border-0 bg-transparent text-sm font-semibold text-[var(--color-brand-dark)]" onClick={() => replaceAvailability(lastCopy.previous)}>Deshacer copia</button>
        </div> : null}
        {availability.length === 0 ? <p className="mt-4 text-sm text-[var(--color-muted)]" role="status">Agregá al menos un bloque de atención para guardar.</p> : null}

        {fieldErrors.availability && dayErrors.size === 0 ? (
