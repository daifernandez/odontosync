...
function MinuteInput({
  ...
  return (
    <label className="block max-w-48 text-sm font-medium text-[var(--color-muted)]">
      {label}
      <span className="relative mt-2 block">
        <input
          ...
          value={value}
        />
        <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-sm font-semibold text-[var(--color-muted)]">
          min
        </span>
      </span>
    </label>
...
export function AgendaPreferencesForm({
  ...
  const { value: preferences, setDraft, discardDraft, restored } = useConfigurationDraft("agenda", {
    gridInterval: String(initialPreferences.gridIntervalMinutes),
    duration: String(initialPreferences.defaultAppointmentDurationMinutes),
    cleanup: String(initialPreferences.defaultCleanupMinutes),
  }, state);
  const { isDirty, markDirty, clearDirty } = useUnsavedChanges(state, restored);
  const fieldErrors = isDirty ? state.fieldErrors : {};
  const { gridInterval, duration, cleanup } = preferences;
  const setGridInterval = (value: string) => setDraft({ ...preferences, gridInterval: value });
  const setDuration = (value: string) => setDraft({ ...preferences, duration: value });
  const setCleanup = (value: string) => setDraft({ ...preferences, cleanup: value });
    ...
    >
      <fieldset disabled={pending} className="m-0 min-w-0 border-0 p-0">
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_15rem] xl:items-start">
          <div>
            <section aria-labelledby="grid-step-title">
              ...
              <ConfigurationFieldError
                ...
                message={fieldErrors.gridIntervalMinutes}
              />
            </section>
            ...
            >
              ...
              <div className="mt-4">
                <MinuteInput
                  describedBy={fieldErrors.defaultAppointmentDurationMinutes ? "agenda-duration-error" : undefined}
                  invalid={Boolean(fieldErrors.defaultAppointmentDurationMinutes)}
                ...
                <ConfigurationFieldError
                  ...
                  message={fieldErrors.defaultAppointmentDurationMinutes}
                />
              </div>
            </section>
            ...
            >
              ...
              <div className="mt-4">
                <MinuteInput
                  describedBy={fieldErrors.defaultCleanupMinutes ? "agenda-cleanup-error" : undefined}
                  invalid={Boolean(fieldErrors.defaultCleanupMinutes)}
                ...
                <ConfigurationFieldError
                  ...
                  message={fieldErrors.defaultCleanupMinutes}
                />
              </div>
            </section>
          </div>
          ...
          >
            <p className="mb-1 text-xs font-bold tracking-[0.1em] text-[var(--color-brand)] uppercase">
              Ejemplo en vivo
            </p>
            <h3 className="m-0 text-lg" id="appointment-example-title">
            ...
            {appointmentEndsAt !== null && nextAppointmentAt !== null ? (
              <div className="mt-4" aria-live="polite">
                <div className="flex gap-3">
                  <span className="mt-0.5 text-sm font-bold text-[var(--color-brand-dark)]">09:00</span>
                ...
                {cleanupMinutes !== null && cleanupMinutes > 0 ? (
                  <div className="mt-2 flex gap-3">
                    <span className="mt-0.5 text-sm font-medium text-[var(--color-muted)]">
                ...
                <div className="mt-4 rounded-xl border border-[var(--color-border)] bg-white p-3 text-center">
                  ...
                  <p className="mt-2 mb-1 text-sm text-[var(--color-muted)]">
        ...
        <ConfigurationSaveBar onDiscard={() => { discardDraft({gridInterval: String(initialPreferences.gridIntervalMinutes), duration: String(initialPreferences.defaultAppointmentDurationMinutes), cleanup: String(initialPreferences.defaultCleanupMinutes)}); clearDirty(); }}
          isDirty={isDirty}
